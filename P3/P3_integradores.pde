...
void draw()
{
  ...
  if (shooting)
  {
    for (int i = 0; i < substeps; i++)
    {
      ...
      //Analytical
      ballXAnalytical += h * ballVXAnalytical;
      ballYAnalytical += h * ballVYAnalytical + 0.5 * h * h * ballG;
      ballVYAnalytical += h * ballG;

      //Explicit
      ballXExplicit += h * ballVXExplicit;
      ballYExplicit += h * ballVYExplicit;
      ballVYExplicit += h * ballG;
      
      //Midpoint
      ballXMidpoint += h * ballVXMidpoint;
      ...
      if (firstStep)
      {
        ...
        ballXVerlet += h * ballVXVerlet;
        ballYVerletOld = ballYVerlet;
        ballYVerlet += h * ballVYVerlet + 0.5 * h * h * ballG;
        firstStep = false;
      }
      else
      {
    ...
    //COLLISION DETECTION
    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    if (ballX > wallX && ballX < (wallX + wallW) && ballY > 0 && ballY < gapY)
    {
      shooting = false;
    }
    else if (ballX > wallX && ballX < (wallX + wallW) && ballY > (gapY + gapH) && ballY < high)
    {
      shooting = false;
    }
