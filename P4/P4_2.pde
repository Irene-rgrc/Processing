...
void draw()
{
  ...
  for (int step = 0; step < substeps; step++)
  {
    ...
    for (int i=0; i<nSprings; i++)
    {
      PVector force = StringElasticForce(nodePos[springNodeA[i]], nodePos[springNodeB[i]], springLenght0[i], springStiffness);
      force.add(SpringDampingForce(nodePos[springNodeA[i]], nodePos[springNodeB[i]], nodeVel[springNodeA[i]], nodeVel[springNodeB[i]]);
      nodeForce[springNodeA[i]].add(force);
      nodeForce[springNodeB[i]].add(force);
    }
